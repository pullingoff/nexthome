    ...
    <div className="relative flex flex-col overflow-hidden h-full border border-medium-brown z-0 rounded-lg hover:-translate-y-2 hover:shadow-[10px_10px_lightgray] hover:duration-200">
      ...
      <section className="flex flex-wrap ml-5 mr-auto mt-0 mb-4">
        {tags?.map((text: string) => (
          <Link
            className="text-[white] uppercase text-[0.85rem] font-semibold bg-deep-brown rounded ml-0 mr-2 mt-0 mb-2 px-2 py-0.5 hover:text-deep-brown hover:bg-[initial] hover:shadow-deep-brown"
            key={text}
            href={`/tags/${text}/1`}
          >
            {text.split(" ").join("-")}
