...
setState는 컴포넌트를 바로바로 리렌더링하지 않고, 변경 사항을 대기열에 넣어두고 한번에 일괄적으로 갱신합니다. 이 결과 setState을 여러번 호출해도 동일한 키에 대해서는 가장 마지막 값으로만 상태가 갱신되기도 합니다.

### 왜 굳이?

이런 setState의 특징 때문에 아무 생각없이 `setState = 상태 바꿔주는 함수` 로만 알고 있으면 뒤통수를 맞기 쉽상입니다. 왜 리액트는 이벤트핸들러 안에서 setState를 비동기적으로 작동하게 했을까요?
...
두번째 방법은 useState 훅을 사용하지 않고, 클래스형 컴포넌트의 setState를 사용하는 방법입니다.  
클래스형 컴포넌트의 setState은 **두번째 인자(`arg2`)로 콜백함수**를 받아서 setState가 실행된 이후 콜백 함수를 실행합니다.
