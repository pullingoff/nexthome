...
2. **padding box**: content box를 둘러쌈. 이때 padding이란 box의 내부를 의미. 
만약 박스가 overflow: auto나 scroll로 돼있으면 이 부분에도 스크롤 바가 생긴다.
...
- inline 요소는 block margin을 가지나 다른 요소들이 그렇게 고려하지 않음. 
inline-block을 사용하면 inline요소의 대부분의 특징을 갖지만 다른 요소들이 block margin을 고려해줌. **inline, inline-block 요소는 내용물 만큼의 크기**를 가진다.
...
padding이나 border값은 별개로 붙는다.(실제 border edge까지의 박스 너비는 정해진 값보다 커진다.)
